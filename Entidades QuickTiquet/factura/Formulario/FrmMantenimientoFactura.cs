...
namespace SFG
{
    public partial class FrmMantenimientoFactura : Bases.MantenimientoBase
    {
        ...
        Modelo.Factura Factura = new Factura();

                protected override void MantenimientoAutomatico(Object sender, EventArgs e)
                {
                    if (!Tools.LlenarEntidadDesdeControlesPersonalizados(this, tabPageRegistro, Factura).Item2)
                    {
                        if (Nuevo)
                        {
                            Sentencia = Factura.Sentencia(Factura, "I");
                            if (Sentencia.Item2)
                            {
                                txtd1cod_fac.Text = Sentencia.Item1.ToString();
                            }
                        }
                        else
                        {
                            Sentencia = Factura.Sentencia(Factura, "U");
                        }
                ...
                protected override void Eliminar(Object sender, EventArgs e)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Factura.cod_fac = Convert.ToInt32(txtd1cod_fac.Text);
                            Sentencia = Factura.Sentencia(Factura, "D");
        ...
        private void FrmMantenimientoFactura_Load(object sender, EventArgs e)
        {
            PrimerControlEdicion = dateCustom2fec_fac;
            UltimoControlEdicion = txtd9tot_fac;
...
/*AddEventsOnLoad*/
        ...
        private void txtd9tot_fac_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 9)
            {
                e.Handled = false;
        ...
        private void txtd9tot_fac_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
                ...
                public void BuscarPor()
        {
            ...
            {
                try { dataGridViewNormal2.DataSource = Factura.ListadoGeneral(txtParametro.Text.Trim()); } catch (Exception exc) { MessageBox.Show(exc.Message); }
        ...
        public void MostrarTodo(object sender = null, EventArgs e = null)
        {
            try
            {
                dataGridViewNormal2.DataSource = Factura.ListadoGeneral();
                lblCantidad.Text = "Items: " + dataGridViewNormal2.Rows.Count.ToString();
        ...
        public void CargarRegistro(string prm_id)
        {
            ...
            ActivarYDesactivarBotonesPorGrupo("1", true);
            btnModificar.Focus();
            DataTable DT = Factura.SFactura(prm_id);
