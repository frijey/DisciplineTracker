...
namespace SFG
{
    public partial class FrmMantenimientoIngresos : Bases.MantenimientoBase
    {
        public FrmMantenimientoIngresos()
        {
            InitializeComponent();
        }

        Modelo.Ingresos Ingresos = new Ingresos();

                protected override void MantenimientoAutomatico(Object sender, EventArgs e)
                {
                    if (!Tools.LlenarEntidadDesdeControlesPersonalizados(this, tabPageRegistro, Ingresos).Item2)
                    {
                        if (Nuevo)
                        {
                            Sentencia = Ingresos.Sentencia(Ingresos, "I");
                            if (Sentencia.Item2)
                            {
                                txtd1cod_ing.Text = Sentencia.Item1.ToString();
                            }
                        }
                        else
                        {
                            Sentencia = Ingresos.Sentencia(Ingresos, "U");
                        }
                ...
                protected override void Eliminar(Object sender, EventArgs e)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Ingresos.cod_ing = Convert.ToInt32(txtd1cod_ing.Text);
                            Sentencia = Ingresos.Sentencia(Ingresos, "D");
        ...
        private void FrmMantenimientoIngresos_Load(object sender, EventArgs e)
        {
            PrimerControlEdicion = txtd2fk_cod_fac;
            UltimoControlEdicion = txtd7bal_pen_ing;
...
/*AddEventsOnLoad*/
        ...
        private void txtd7bal_pen_ing_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 9)
            {
                e.Handled = false;
        ...
        private void txtd7bal_pen_ing_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
                ...
                public void BuscarPor()
        {
            ...
            {
                try { dataGridViewNormal2.DataSource = Ingresos.ListadoGeneral(txtParametro.Text.Trim()); } catch (Exception exc) { MessageBox.Show(exc.Message); }
        ...
        public void MostrarTodo(object sender = null, EventArgs e = null)
        {
            try
            {
                dataGridViewNormal2.DataSource = Ingresos.ListadoGeneral();
                lblCantidad.Text = "Items: " + dataGridViewNormal2.Rows.Count.ToString();
        ...
        public void CargarRegistro(string prm_id)
        {
            ...
            ActivarYDesactivarBotonesPorGrupo("1", true);
            btnModificar.Focus();
            DataTable DT = Ingresos.SIngresos(prm_id);
