...
namespace SFG
{
    public partial class FrmMantenimientoUsuario : Bases.MantenimientoBase
    {
        ...
        Modelo.Usuario Usuario = new Usuario();
                ...
                {
                    if (!Tools.LlenarEntidadDesdeControlesPersonalizados(this, tabPageRegistro, Usuario).Item2)
                    {
                        if (Nuevo)
                        {
                            Sentencia = Usuario.Sentencia(Usuario, "I");
                            if (Sentencia.Item2)
                            {
                                txtd1cod_usu.Text = Sentencia.Item1.ToString();
                            }
                        }
                        else
                        {
                            Sentencia = Usuario.Sentencia(Usuario, "U");
                        }
                ...
                protected override void Eliminar(Object sender, EventArgs e)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Usuario.cod_usu = Convert.ToInt32(txtd1cod_usu.Text);
                            Sentencia = Usuario.Sentencia(Usuario, "D");
        ...
        {
            PrimerControlEdicion = txtd2nom_usu;
            UltimoControlEdicion = txtd6fk_rol;
...
/*AddEventsOnLoad*/
        ...
        private void txtd6fk_rol_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 9)
            {
                e.Handled = false;
        ...
        private void txtd6fk_rol_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
                ...
                public void BuscarPor()
        {
            ...
            {
                try { dataGridViewNormal2.DataSource = Usuario.ListadoGeneral(txtParametro.Text.Trim()); } catch (Exception exc) { MessageBox.Show(exc.Message); }
        ...
        public void CargarRegistro(string prm_id)
        {
            ...
            ActivarYDesactivarBotonesPorGrupo("1", true);
            btnModificar.Focus();
            DataTable DT = Usuario.SUsuario(prm_id);
